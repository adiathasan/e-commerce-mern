...
const ProfileScreen = () => {
	...
	const singleStore = user?.store?.[0]?._id;

	React.useEffect(() => {
		if (singleStore) {
			dispatch(getSingleStore(singleStore));
		}
	...
	return isLoading ? (
		...
		<Row className='justify-content-md-center'>
			...
			<Col md={3}>
				...
				<h2 className='mt-3'>User Profile</h2>
				...
				<Form onSubmit={handleSubmit}>
					<Form.Group>
						...
						<div className='d-flex align-items-center justify-content-center'>
							...
							{showConfirmPassword ? (
								<VisibilityOffIcon onClick={handleShowConfirmPassword} />
							) : (
								<VisibilityIcon onClick={handleShowConfirmPassword} />
							)}
						</div>
							...
							{orders.map((order) => {
								return (
									<tr key={order._id}>
										...
										<td>
											{order.isPaid ? order.paidAt.substring(0, 10) : <ClearOutlinedIcon style={{ color: 'red' }} />}
										</td>
										<td>
											{order.isDelivered ? (
